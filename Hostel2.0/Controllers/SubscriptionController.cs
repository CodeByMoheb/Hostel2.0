...
namespace Hostel2._0.Controllers
{
    ...
    public class SubscriptionController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditPlan(string id, SubscriptionPlan model)
        {
            if (id != model.Id)
            {
                return NotFound();
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeletePlanConfirmed(string id)
        {
            var plan = await _context.SubscriptionPlans.FindAsync(id);
            if (plan == null)
            {
            ...
            var inUse = await _context.HostelSubscriptions
                .AnyAsync(hs => hs.SubscriptionPlanId == id && hs.Status == SubscriptionStatus.Active);
